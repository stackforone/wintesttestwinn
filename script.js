function winNumber() {
    ...
    let results = generateCombinations(input);
    displayResults(results, excludeDouble);
}

function reverseWinNumber() {
    const input = document.getElementById('input-number').value;
    const excludeDouble = document.getElementById('exclude-checkbox').checked;
    let results = generateCombinations(input.split('').reverse().join(''));
    displayResults(results, excludeDouble);
}
...
function displayResults(results, excludeDouble) {
    const resultContainer = document.getElementById('result');
    resultContainer.innerHTML = '';

    const set1 = filterResults(results, 3, false, excludeDouble);
    const set2 = filterResults(results, 2, false, excludeDouble);
    const set3 = filterResults(results, 3, true, excludeDouble);
    const set4 = filterResults(results, 2, true, excludeDouble);
...
function filterResults(results, length, includeDouble, excludeDouble) {
    return results.filter(result => {
        if (result.length === length) {
            if (excludeDouble && (result[0] === result[1] || (length === 3 && (result[1] === result[2] || result[0] === result[2])))) {
                return false;
            }
            if (includeDouble) {
